report 50110 "Move Reverse Entries"
{
    ...
    local procedure Token(VAR Text: Text[1024];
    Separator: Text[2]) Token: Text[1024]
    var
        Pos: Integer;
    begin
    ...
    local procedure CheckAndCreateBatch(CompanyName: Text;
    ...
    begin
        ...
        ismatched := '';
        if companywiseproject.FindSet() then
            repeat
      ...
      */
        ...
        if Project.Contains('3DC|GENERAL') then Project := 'GENERAL';
...
#pragma warning restore AL0432
    ...
    var IsSetupInProgress: Boolean)
    begin
    end;

    local procedure CheckColumnLayout(Name: Text[10];
    FromAccSched: Record "Acc. Schedule Name";
    company: Text) isExist: Boolean
    var
    ...
    begin
        ...
        if (ColLayoutName.Get(FromAccSched."Default Column Layout")) then begin
            isExist := true;
            //CreateColumnLayout()
        end
        else begin
    ...
    local procedure CreateColumnLayoutName(Name: Text;
    FromColLayout: Record "Column Layout Name";
    Company: Text)
    var
        ColLayoutName: Record "Column Layout Name";
    begin
        ColLayoutName.ChangeCompany(Company);
        if ColLayoutName.Get(Name) then exit;
        ColLayoutName.Init();
    ...
    local procedure CreateNewAccountScheduleName(NewName: Code[10];
    ...
    begin
        ...
        if AccScheduleName.Get(NewName) then exit;
    ...
    local procedure CreateColumnLayout(NewName: Text;
    FromColLayout: Record "Column Layout";
    company: text)
    var
        ColLayout: Record "Column Layout";
    begin
        ColLayout.ChangeCompany(company);
        if ColLayout.Get(NewName, FromColLayout."Line No.") then exit;
        ColLayout.Init();
    ...
    local procedure CreateNewAccountScheduleLine(NewName: Code[10];
    ...
    begin
        ...
        if AccScheduleLine.Get(NewName, FromAccScheduleLine."Line No.") then exit;
